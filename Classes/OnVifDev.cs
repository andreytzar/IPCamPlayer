...
namespace IPCamPlayer.Classes
{
    ...
    {
        public List<MediaToken> MediaTokens { get; private set; } = new();
        ...
        //List<Profile> _mediaProfiles  = new();
        ...
        internal async Task<bool> Connect(string url, SecureString password, string login = "", BasicHttpSecurityMode mode = BasicHttpSecurityMode.TransportCredentialOnly, HttpClientCredentialType credentialType = HttpClientCredentialType.Digest)
        {
            ...
            try
            {
                ...
                Status?.Invoke(this, $"OnVif Device Time: {t.LocalDateTime.Date.Month}/{t.LocalDateTime.Date.Day} {t.LocalDateTime.Time.Hour}:{t.LocalDateTime.Time.Minute}");
                ...
                if (await GetServices())
                {
                    ...
                    if (serviceMedia != null) 
                    {
                        ...
                        if (result != null)
                        {
                            Status?.Invoke(this, "Medis Service:");
                            foreach (var prof in result.Profiles)
                            {
                                ...
                                {
                                    MediaTokens.Add(new() { Name=prof.Name, Rtsp = rtsp, Token=prof.token, SnapShot=snapshot });
                    ...
                    if (serviceMedia != null)
                    {
                        _media2 = new Media2Client(bind, new(serviceMedia.XAddr));
